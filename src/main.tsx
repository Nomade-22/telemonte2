...
function App(){
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-user"><small>{user.role.toUpperCase()}</small><strong>{user.name}</strong><button onClick={()=>setUser(null)}>Sair</button></div>
    </aside>
    <main>
      ...
      <section className="content">
        ...
        {(page==='Área do Motorista'||page==='Minha Rota') && <DriverArea db={db} setDB={setDB}/>} 
...
function Login({onLogin}:{onLogin:(u:User)=>void}){
  const [email,setEmail]=useState('admin@telemonte.local'); const [password,setPassword]=useState('admin123'); const [error,setError]=useState('');
  const users=[
    {email:'admin@telemonte.local',password:'admin123',name:'Administrador',role:'admin' as Role},
    {email:'supervisor@telemonte.local',password:'super123',name:'Supervisor Operacional',role:'supervisor' as Role},
    {email:'motorista@telemonte.local',password:'rota123',name:'João Silva',role:'driver' as Role}
  ];
  function submit(e:React.FormEvent){e.preventDefault();const u=users.find(x=>x.email===email&&x.password===password); if(!u){setError('E-mail ou senha inválidos.');return} onLogin({email:u.email,name:u.name,role:u.role});}
  return <div className="login-page"><div className="login-card"><div className="login-logo">T</div><h1>Telemonte</h1><p>Sistema de Gestão de Coleta de Resíduos</p><form onSubmit={submit}><label>E-mail<input value={email} onChange={e=>setEmail(e.target.value)} type="email"/></label><label>Senha<input value={password} onChange={e=>setPassword(e.target.value)} type="password"/></label>{error&&<div className="error">{error}</div>}<button className="primary" type="submit">Entrar</button></form><div className="demo"><strong>Acessos de demonstração</strong><span>Admin: admin@telemonte.local / admin123</span><span>Motorista: motorista@telemonte.local / rota123</span></div></div></div>
}
...
function CrudRoutes({db,setDB}:{db:DB;setDB:React.Dispatch<React.SetStateAction<DB>>}){
 ...
 function add(e:React.FormEvent){e.preventDefault();if(!form.code)return;setDB(d=>({...d,routes:[...d.routes,{id:uid('r'),code:form.code,date:form.date,vehicle:form.vehicle,driver:form.driver,clients:Number(form.clients)||1,done:0,status:'Programada'}]}));}
...
function DriverArea({db,setDB}:{db:DB;setDB:React.Dispatch<React.SetStateAction<DB>>}){
 const route=db.routes.find(r=>r.driver.toLowerCase().includes('joão'))||db.routes[0]; const [log,setLog]=useState<string[]>(()=>JSON.parse(localStorage.getItem('driver-log')||'[]')); const [gps,setGps]=useState('Localização ainda não registrada');
